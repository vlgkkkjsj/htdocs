...
if ($qtd > 0) {
    ...
    foreach ($res as $row) {
       ...
       echo "<td>
            ...
            <button onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=exclui_voluntariosr&acao=excluir&id=" . $row->getId() . "';}else{false;}\" class='excluir'>Excluir</button>
            </td>";
        echo "</tr>";
    }
    echo "</table>";
